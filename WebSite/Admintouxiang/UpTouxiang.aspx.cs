...
//引入命名空间
public partial class Admintouxiang_Default : Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (type == "bmp" || type == "BMP" || type == "jpg" || type == "JPG" || type == "gif" || type == "GIF")
        {
            ...
            try
            {
                LblImage.Text = "~/phone/" + LblImage.Text;
                String SqlInsert = "Insert tb_image values ('" + LblImage.Text + "','" + DateTime.Now.ToString() + "')";
                EP.EXECCommand(SqlInsert);
                Response.Write(
                    "<script lanuage='javaScript'>alert('恭喜您！！添加成功！');location='javascript:history.go(-1)'</script>");
                //弹出对话框显示“恭喜您！！修改成功！”
            }
            catch
            {
                Response.Write(
                    "<script lanuage='javaScript'>alert('很遗憾！！添加失败！');location='javascript:history.go(-1)'</script>");
                //弹出对话框显示“很遗憾！！修改失败！”
            }
        }
        else
